...
\begin{document}
	...
	\subsection{Cơ chế Fitness Shaping}
	Một đặc điểm quan trọng của xNES là không sử dụng trực tiếp giá trị hàm mục tiêu $f(x_i)$. Thay vào đó, thuật toán sử dụng cơ chế \textbf{xếp hạng (rank-based fitness shaping)}. 
	
	Các cá thể được sắp xếp dựa trên độ thích nghi, và trọng số $u_i$ được gán dựa trên thứ hạng $k$ của cá thể đó:
	\begin{equation}
		u_k = \frac{\max(0, \log(\lambda/2 + 1) - \log(k))}{\sum_{j=1}^\lambda \max(0, \log(\lambda/2 + 1) - \log(j))} - \frac{1}{\lambda}.
	\end{equation}
	Việc này giúp thuật toán bất biến với các phép biến đổi đơn điệu của hàm mục tiêu và hạn chế tác động của các giá trị ngoại lai (outliers).
	...
	Chúng tôi tiến hành thử nghiệm với các tham số cấu hình như sau:
	...
	\begin{table}[H]
		...
		\begin{tabular}{lc}
			...
			\midrule
			Số chiều ($d$) & 2 \\
			Kích thước quần thể ($\lambda$) & 50 \\
			Số vòng lặp (Iterations) & 60 \\
			Learning rate vị trí ($\eta_\mu$) & 1.0 \\
			Learning rate hình dạng ($\eta_\sigma, \eta_B$) & $\approx 0.783$ \\
			Khởi tạo $\mu$ & $(4.0, 4.0)$ \\
			Khởi tạo $\sigma$ & 1.0 \\
			\bottomrule
		\end{tabular}
		\caption{Cấu hình tham số thực nghiệm cho thuật toán xNES}
		\label{tab:params}
	\end{table}
	...
	\begin{figure}[H]
		...
		\caption{So sánh quỹ đạo hội tụ (phần 1)}
		\label{fig:xnesvsother-1}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\caption{So sánh quỹ đạo hội tụ: xNES và CMA-ES tìm được cực trị toàn cục (0,0), trong khi Gradient Descent bị kẹt tại cực trị địa phương}
	...
	\begin{thebibliography}{9}
		...
		\textit{Natural evolution strategies}.
		The Journal of Machine Learning Research, 15(1), 949-980.
		...
		\bibitem{amari1998}
		Amari, S. I. (1998).
		\textit{Natural gradient works efficiently in learning}.
